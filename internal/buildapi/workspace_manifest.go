...
// maxWorkspaceHydrateBytes bounds the serialized workspace-hydrate annotation.
// Kubernetes caps the total size of all annotation keys and values on an object
// at 256 KiB (TotalAnnotationSizeLimitB). Stay well under it so the trace-id,
// requested-by and task-bundle annotations still fit alongside the plan.
const maxWorkspaceHydrateBytes = 220 * 1024

// ErrWorkspaceHydratePlanTooLarge is returned when the workspace hydrate plan
// would not fit within the annotation size limit. Callers should surface it as
// a client error rather than a 500.
