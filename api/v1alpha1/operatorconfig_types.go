...
type JWTIssuerConfig struct {
	...
	// PEM-encoded CA certificate. The operator reads the secret at reconcile time
	// and on each 60-second auth refresh, so CA rotations are picked up automatically.
	// +optional
	...
	// PEM-encoded CA certificate. The operator reads the ConfigMap at reconcile time
	// and on each 60-second auth refresh, so CA rotations are picked up automatically.
	// +optional
