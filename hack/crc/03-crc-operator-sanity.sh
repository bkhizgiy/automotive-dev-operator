...
if [[ -z "${KUBECONFIG:-}" && -f "$HOME/.crc/machines/crc/kubeconfig" ]]; then
...
echo -e "  ${CYAN}INFO${NC}  Waiting for Build API pod to be ready (up to 2 min)..."
for _ in {1..24}; do
    POD_STATUS=$(oc get pods -n "$NAMESPACE" -l app.kubernetes.io/component=build-api --no-headers 2>/dev/null || echo "")
    if echo "$POD_STATUS" | grep -q "2/2.*Running"; then
        break
    fi
    sleep 5
done
...
if [[ "${1:-}" == "--sanity" ]]; then
    ...
    if [[ ! -f "$CAIB_BIN" ]]; then
        ...
        if ! make -C "$REPO_DIR" build-caib 2>&1 | tail -10; then
            echo -e "  ${CYAN}INFO${NC}  make build-caib did not succeed; will check if binary exists."
        fi
    fi
